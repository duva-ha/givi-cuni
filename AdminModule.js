...
const AdminModule = ({ students = [], logout, db, weeklyFees = {} }) => {
    ...
    // 1. CHỨC NĂNG XUẤT DỮ LIỆU (Copy báo cáo nhanh)
    const exportToZalo = () => {
        const title = `📊 BÁO CÁO THI ĐUA LỚP 10A7\n━━━━━━━━━━━━━━━\n`;
        const content = filteredStudents.map((s, i) => 
            `${i + 1}. [Tổ ${s.group}] ${s.name} - ${s.totalXp || 0} XP (Lv.${s.level || 1})`
        ).join("\n");
        
        navigator.clipboard.writeText(title + content);
        alert("✅ Đã copy báo cáo thi đua! Bạn có thể dán vào Zalo ngay.");
    };

    // 2. LOGIC LỌC THEO TỔ & TÌM KIẾM
    ...
    const handleAdd = () => {
        ...
        if (!name || !group) return alert("Thiếu tên hoặc tổ!");
    ...
    const handleDelete = (id, name) => {
        if (window.confirm(`Xóa ${name}?`)) db.collection("students").doc(id).delete();
    };

    return (
        <div className="max-w-lg mx-auto p-4 pb-24 animate-fadeIn">
            {/* Header với nút Xuất Dữ Liệu */}
            <header className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-black italic text-indigo-400 leading-none">ADMIN 10A7</h1>
                    <p className="text-[9px] font-bold opacity-30 mt-1 uppercase tracking-tighter">Quản lý lớp học chuyên sâu</p>
                </div>
                <div className="flex gap-2">
                    <button onClick={exportToZalo} className="bg-green-600/20 text-green-500 px-3 py-2 rounded-xl text-[10px] font-black border border-green-500/20 active:scale-95 transition-all">
            ...
            {/* BỘ LỌC TỔ (Dạng tab trượt) */}
            <div className="flex gap-2 mb-6 overflow-x-auto no-scrollbar pb-2 border-b border-white/5">
            ...
            <div className="glass p-5 rounded-[2rem] mb-8 border-dashed border-2 border-indigo-500/20 bg-indigo-500/5">
                <input id="nameInp" placeholder="NHẬP HỌ TÊN..." className="w-full p-4 glass rounded-xl mb-3 text-xs font-black uppercase outline-none text-white border border-white/5" />
                <div className="flex gap-3">
                    ...
                    <button onClick={handleAdd} className="hero-gradient px-8 rounded-xl font-black text-[10px] uppercase shadow-lg active:scale-95 transition-all">THÊM +</button>
                </div>
            </div>
                ...
                {filteredStudents.map(s => (
                    <div key={s.id} className="glass p-4 rounded-[2rem] flex justify-between items-center border border-white/5">
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={() => db.collection("students").doc(s.id).update({totalXp: (s.totalXp||0)+10, level: Math.floor(((s.totalXp||0)+10)/500)+1})} className="w-9 h-9 bg-blue-500/10 text-blue-400 rounded-xl text-[9px] font-black border border-blue-500/20">HT</button>
                            <button onClick={() => db.collection("students").doc(s.id).update({totalXp: (s.totalXp||0)+30, level: Math.floor(((s.totalXp||0)+30)/500)+1})} className="w-9 h-9 bg-green-500/10 text-green-400 rounded-xl text-[9px] font-black border border-green-500/20">LĐ</button>
                            <button onClick={() => handleDelete(s.id, s.name)} className="w-9 h-9 text-red-900/20 hover:text-red-500 transition-all"><i className="fa-solid fa-trash-can text-xs"></i></button>
                        </div>
                    </div>
                ))}
            </div>
